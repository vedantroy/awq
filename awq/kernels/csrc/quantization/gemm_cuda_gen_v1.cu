...
__host__ int test_divide_round_up(int c, int divisor){
  // Same thing as ceil(float(c) / float(divisor))
  return (c + divisor - 1) / divisor;
}
...
__global__ void __launch_bounds__(128) 
  gemm_forward_4bit_cuda_m128n64k32
  (
  int split_k_iters, 
    ...
    int split_k_iters)
{
    ...
    assert(exp_num_blocks == test_divide_round_up(num_out_feats, 128) * (num_out_channels / 64) * split_k_iters);
